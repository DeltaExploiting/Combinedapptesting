...
struct SigningSettings:View{@AppStorage("signingURL")var url="https://flarestore.app/api/sign";@AppStorage("p12Name")var p12Name="";@AppStorage("provName")var provName="";@State var pass="";@State var p12=false;@State var prov=false;@Environment(\.dismiss)var dismiss;let dir=FileManager.default.urls(for:.applicationSupportDirectory,in:.userDomainMask)[0].appendingPathComponent("DualIPA/SigningAssets",isDirectory:true);var body:some View{NavigationStack{Form{Section("Signing API"){TextField("API URL",text:$url).keyboardType(.URL).textInputAutocapitalization(.never);Text("Use an authorized signing endpoint that accepts your IPA, P12, provisioning profile, and password.").font(.footnote).foregroundStyle(.secondary)};Section("Authorized signing assets"){Button("Import P12 \(p12Name.isEmpty ? "":"✓")"){p12=true};Button("Import mobileprovision \(provName.isEmpty ? "":"✓")"){prov=true};SecureField("P12 password (optional)",text:$pass)};Text("The P12/profile are uploaded only when you explicitly press Sign & Launch.").font(.footnote).foregroundStyle(.secondary)}.navigationTitle("Signing Service").toolbar{Button("Done"){KC.s.set(pass);dismiss()}}.sheet(isPresented:$p12){Picker(exts:["p12","pfx"],picked:{u in p12=false;save(u.first,to:"signing.p12");p12Name=u.first?.lastPathComponent ?? ""},cancel:{p12=false})}.sheet(isPresented:$prov){Picker(exts:["mobileprovision"],picked:{u in prov=false;save(u.first,to:"signing.mobileprovision");provName=u.first?.lastPathComponent ?? ""},cancel:{prov=false})}.onAppear{pass=KC.s.get()}}}
...
struct ContentView:View{@AppStorage("appleSignedIn")var apple=false;@AppStorage("enterpriseMode")var enterprise=false;@AppStorage("signingURL")var signingURL="https://flarestore.app/api/sign";@State var apps=[GuestApp]();@State var imp=false;@State var certs=false;@State var settings=false;@State var selected:Set<String>=[];@State var app:GuestApp?;@State var msg="";@State var signing=false;let dir=FileManager.default.urls(for:.applicationSupportDirectory,in:.userDomainMask)[0].appendingPathComponent("DualIPA",isDirectory:true)
 ...
 var library:some View{NavigationStack{Group{if apps.isEmpty{ContentUnavailableView("No Apps",systemImage:"square.stack.3d.up",description:Text("Import an IPA to begin."))}else{List(apps){x in Button{xSelected(x)}label:{HStack{Image(systemName:"app.fill");Text(x.displayName);Spacer();Text("Launch").foregroundStyle(.tint)}}.buttonStyle(.plain)}.onDelete{idx in apps.remove(atOffsets:idx);save()}}}.navigationTitle("Dual IPA").toolbar{
   ToolbarItem(placement:.topBarTrailing){Button{imp=true}label:{Image(systemName:"plus")}}
   ToolbarItem(placement:.topBarTrailing){Button{certs=true}label:{Label("Certificates",systemImage:"checkmark.seal")}.opacity(enterprise ? 1 : 0)}.disabled(!enterprise)
   ToolbarItem(placement:.topBarTrailing){Button{settings=true}label:{Image(systemName:"server.rack")}.opacity(enterprise ? 1 : 0)}.disabled(!enterprise)
   ToolbarItem(placement:.topBarTrailing){Button("Sign Out"){apple=false;enterprise=false}}
 }.sheet(isPresented:$imp){IPAPicker(picked:{u in imp=false;importIPAs(u)},cancel:{imp=false})}.sheet(isPresented:$certs){Certs(selected:$selected)}.sheet(isPresented:$settings){SigningSettings()}.sheet(item:$app){Launch(app:$0,enterprise:enterprise,certs:selected,url:signingURL,signing:$signing){msg=$0}}.alert("Dual IPA",isPresented:Binding(get:{!msg.isEmpty},set:{if !$0{msg=""}})){Button("OK"){} }message:{Text(msg)}}}
    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 18) {
                ...
                if enterprise {
                    Text(certs.count == 1 ? "Certificate selected" : "Select exactly one certificate").foregroundStyle(certs.count == 1 ? Color.secondary : Color.orange)
